...
import { LoadingState, LayoutGroup, TABS } from "../types/index";
...
interface GenerateButtonProps {
    ...
    themesStreaming?: boolean;
    themesLoading?: boolean;
    showGoToPresentation?: boolean;
...
const GenerateButton: React.FC<GenerateButtonProps> = ({
    ...
    onSubmit,
    onGenerateThemes,
    themesStreaming = false,
    themesLoading = false,
    showGoToPresentation = false,
    onGoToPresentation,
    activeTab
}) => {
    const pathname = usePathname();

    const isLoading = loadingState.isLoading || streamState.isLoading || streamState.isStreaming;
    const hasTemplate = !!selectedLayoutGroup;

    // Debug logs solo cuando hay cambios
    useEffect(() => {
        console.log("GenerateButton State:", {
            activeTab,
            isLoading,
            hasTemplate,
    ...
    }, [activeTab, isLoading, hasTemplate, selectedLayoutGroup]);
    
    const getButtonText = () => {
        if (loadingState.isLoading && loadingState.message) return loadingState.message;
        if (streamState.isLoading || streamState.isStreaming) return "Loading...";
        if (!hasTemplate) return "Seleccionar Plantilla";
        return "Generar Presentación";
    };

    const getButtonColor = () => {
        if (isLoading) return 'bg-gray-400';
        if (hasTemplate) return 'bg-[#5146E5] hover:bg-[#5146E5]/80';
    ...
    return (
        <div className="flex gap-4 w-full">
            {/* Temas Button */}
            <Button
                disabled={themesLoading || themesStreaming}
                style={{ opacity: activeTab !== TABS.THEMES ? 0.5 : 1 }}
                onClick={() => {
                    if (!themesLoading && !themesStreaming && onGenerateThemes) {
                        trackEvent("outline_generate_themes_button_clicked", { pathname });
                        onGenerateThemes();
                    }
                }}
                className="bg-[#10B981] flex-1 rounded-lg text-base sm:text-lg py-4 sm:py-6 font-instrument_sans font-semibold hover:bg-[#10B981]/80 text-white disabled:opacity-50 disabled:cursor-not-allowed"
            >
                <svg
                    className="mr-2"
                    width="24"
                ...
                {themesLoading || themesStreaming ? "Generando..." : "Temas"}
            </Button>

            {/* Ir a la presentación Button - En tab de temas y layouts */}
            {showGoToPresentation && (activeTab === TABS.THEMES || activeTab === TABS.LAYOUTS) && (
                <Button
                    disabled={isLoading || !hasTemplate}
                    onClick={() => {
                        if (!isLoading && hasTemplate && onGoToPresentation) {
                            trackEvent("themes_go_to_presentation_clicked", { pathname });
                            onGoToPresentation();
                        }
                    }}
                    className={`flex-1 rounded-lg text-base sm:text-lg py-4 sm:py-6 font-instrument_sans font-semibold text-white disabled:opacity-50 disabled:cursor-not-allowed ${
            ...
            <Button
                ...
                className={`flex-1 rounded-lg text-base sm:text-lg py-4 sm:py-6 font-instrument_sans font-semibold text-white disabled:opacity-50 disabled:cursor-not-allowed ${getButtonColor()}`}
            >
                <svg
                    className="mr-2"
                    width="24"
